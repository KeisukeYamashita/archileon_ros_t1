...
def judge_rob_goal(temporal_world_rob_x, temporal_world_rob_y, world_goal_x, world_goal_y):
    result = False
    diff_x = (world_goal_x - temporal_world_rob_x)
    diff_y = (world_goal_y - temporal_world_rob_y)
    ...
    if cal_counter.num == 1:
        ...
        # print(abs(diff_thr.diff_xy[0]) + abs(diff_thr.diff_xy[1]))
        ...
        if math.sqrt(diff_x**2 + diff_y**2) <= math.sqrt(diff_thr.diff_xy[0]**2 + diff_thr.diff_xy[1]**2):
            ...
            print("diff is " + str(math.sqrt(diff_thr.diff_xy[0]**2 + diff_thr.diff_xy[1]**2)))
        # if abs(diff_x) + abs(diff_y) > abs(diff_thr.diff_xy[0]) + abs(diff_thr.diff_xy[1]):
            # print("/////////////////////////////////////////////////")
            # print("End "  + str(diff_thr.diff_xy[0]))
            # print("End "  + str(diff_thr.diff_xy[1]))
            # print("End")
            # print(" ")
            cal_counter.reset_count()
            result = True
    return result
